...
for binary in sandboxd sandbox-init profile-bundle t05-tests; do
...
exec "${privilege[@]}" unshare --mount --pid --fork --mount-proc --net --propagation private \
  bash -c '
    ...
    mkdir /tmp/t05-bin
    cp -- ./sandboxd ./sandbox-init ./profile-bundle ./t05-tests ./python.bundle /tmp/t05-bin/
    cd /
    ...
    uname -sr
    /tmp/t05-bin/t05-tests -test.v -test.run="^TestSandboxExecution" -test.timeout=180s
